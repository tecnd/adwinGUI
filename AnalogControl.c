...
This panel is used to set the value of an analog control.  It is called by
double-clicking on an analog table cell. Clicking on Set Value, reads the values
from the control and stores them in the AnalogTable.
*/
...
/**
Reads the value of the last active column before this one and displays it on the
panel.
*/
...
If adding another function, just add another case, and modify the ring control
