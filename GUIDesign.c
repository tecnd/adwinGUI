...
- UpdateNum - each entry is the number of channel updates that we perform during
the ADwin EVENT, where an ADwin event is an update cycle, i.e. 10 microseconds,
100 microseconds... etc. We advance through this array once per ADwin Event.
UpdateNum controls how fast we scan through ChNum and ChVal
- ChNum - An array that contains the channel number to be updated. Synchronous
with ChVal. Channels listed below
- ChVal - An array that contains the value to be written to a channel.
Synchronous with ChVal.

- ChNum
        - Value 1-32: Analog lines, 4 cards with 8 lines each.  ChVal is -10V to
10V
        - Value 51: DDS1 line. ChVal is either a 2-bit value (0-3) to write, or
(4-7) a reset signal
        - Value 52: DDS2 line. ChVal is either a 2-bit value (0-3) to write, or
(4-7) a reset signal
        - Value 101, 102 First 16 and last 16 lines on the first DIO card. ChVal
is a 16 bit integer
        - Value 103, 104 First 16 and last 16 lines on the second DIO card.
ChVal is a 16 bit integer
        - ChNum 201, 202 These are codes to enable/disable looping.
Corresponding ChVal is the number of loops.
        .
- dds_cmd_seq - List of dds commands, parsed into 2-bit sections, or reset lines
to be written Commands are listed along with the time they should occur at.
.
*/
        ...
        for (int i = 1; i <= numtimes; i++) {
            ...
            for (int j = 1; j <= NUMBERANALOGCHANNELS; j++) {
                ...
                if (AMat[j][i].fval != AMat[j][i - 1].fval) {
                    ...
                    NewAval = CalcFcnValue(AMat[j][i].fcn, AMat[j][i - 1].fval, AMat[j][i].fval,
            ...
            while (t < NewTimeMat[i] - 1) {
                ...
                while (k < usefcn) {
                    k++;
                    int c = UsingFcns[k];
                    NewAval = CalcFcnValue(AMat[c][i].fcn, AMat[c][i - 1].fval, AMat[c][i].fval,
...
void UpdateScanValue(int Reset)
{
    ...
    {
        if (ScanUp) {
            if ((ScanVal.Current_Value >= ScanVal.End) && (ScanVal.Current_Iteration >= ScanVal.Iterations - 1)) { // Done Scan
...
void SaveSettings(void)
{
    ...
    int status = FileSelectPopupEx("C:\\UserDate\\Data", "*.pan", "", "Save Settings",
    ...
    int eventData2)
{
    ...
    case EVENT_COMMIT:
        for (int i = 1; i <= NUMBEROFCOLUMNS; i++) {
            ...
            for (int j = 1; j <= NUMBERANALOGCHANNELS; j++) {
                tscaleold = AnalogTable[i][j][currentpage]
...
void CVICALLBACK Scan_Table_Load(int panelHandle, int controlID, int MenuItemID,
    void* callbackData)
{
