...
#define CL_COLUMN_LEN 30   // Number of events that can be triggered
 ...
 *************************************************************/
void ioReset()
{  //changed by JH from port PE_0 to PE_7
...
/*******JH changed initialize to send bytes=00000000 and change port for IO_RESET
from 0 to 7*/

   // Clear contents of ramp limit registers
  /*JH sendByte(DR_LIMIT_ADDRESS);
   sendByte(0x00);
   sendByte(0x00);
   sendByte(0x00);
   sendByte(0x00);
   ...
   sendByte(0x00);   // AD9910 default
  //JH sendByte(0x40);   // Autoclear digital ramp accumulator
  //JH sendByte(0x00);   // AD9910 default
...
// sendByte(0x20);   // AD9910 default
  ...
  /**JH sendByte(CFR2_ADDRESS); // Address of CFR2
   sendByte(0x00);   // AD9910 default
...
*/
  ...
  /*JH sendByte(0x02);   // Address of CFR3
   ...
   /*JHsendByte(AUX_DAC_CONTROL_ADDRESS);
   sendByte(0x00);
   sendByte(0x00);
   sendByte(0x00);
   sendByte(0x00);      */

   //JH toggleUpdateClock(); // Transfer all of these commands to the registers of
                        // the DDS

   //JH BitWrPortI(PEDR, &PEDRShadow, 1, 7);
}
      ...
      while(!listen)
      {
         costate
         {
          ...
          //   waitfor(DelayMs(1));          // Waits 1 milliseconds to see if the trigger is released.
            ...
            //JH-changed to send all of the sweeps at once
                  ...
                  */
                  ...
                  if(commandlist[CLrdInd][0])
                  {
                     ...
                     */
                     ...
                     /*JH turned off so only sends data
                     toggleUpdateClock();
